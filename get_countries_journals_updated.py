...
cursor.execute("SELECT name, authors FROM journal_papers")
...
country_papers = {country: [] for country in country_map.values()}

# List to store authors with no country found
authors_no_country = []
...
test_limit = 114767  # Adjust as needed
...
for i, (title, authors_str) in enumerate(valid_papers[:test_limit]):
    ...
    if country_code:
        ...
        if country_code in country_map:
            ...
            country_papers[country_name].append({
                'title': title,
                'author': first_author,
            ...
            print(f" Found country {country_code} (not in target list - not adding to CSV)")
    else:
        # Only add to CSV if no country was found at all
        authors_no_country.append({
            'author': first_author,
            'title': title,
            'all_authors': authors_str
        })
...
print(f"\nCountry-wise paper distribution:")

# Display country lists with paper counts
for country, papers in country_papers.items():
    if papers:
        print(f"\n{country}: {len(papers)} papers")
        print("-" * (len(country) + 20))
        for j, paper in enumerate(papers[:5], 1):  # Show first 5 papers as sample
            safe_title = paper['title'].encode('ascii', 'ignore').decode('ascii')
            safe_author = paper['author'].encode('ascii', 'ignore').decode('ascii')
            print(f"  {j}. {safe_title[:60]}... (Author: {safe_author})")
        if len(papers) > 5:
            print(f"  ... and {len(papers) - 5} more papers")

# Create CSV for authors with no country found
if authors_no_country:
    csv_filename = "authors_no_country_found.csv"
    with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Author Name', 'Paper Title', 'All Authors']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        
        writer.writeheader()
        for entry in authors_no_country:
            writer.writerow({
                'Author Name': entry['author'],
                'Paper Title': entry['title'],
                'All Authors': entry['all_authors']
            })
    
    print(f"\n Created CSV file '{csv_filename}' with {len(authors_no_country)} authors where no country was found")
else:
    print(f"\nAll authors had countries identified (or found countries not in mapping)")

# Summary statistics
print(f"\n" + "="*60)
print(f"FINAL STATISTICS:")
print(f"Total papers processed: {test_limit}")
print(f"Total authors with country found: {successful_lookups}")
print(f"Authors with no country found: {len(authors_no_country)}")
print(f"Authors with countries in target list:")
for country, papers in country_papers.items():
    if papers:
        print(f"  • {country}: {len(papers)}")
